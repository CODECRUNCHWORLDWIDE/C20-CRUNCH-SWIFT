...
```mermaid
flowchart TD
  A1["Passive eavesdropper"] --> D1["TLS and ATS"]
  A2["Active MITM"] --> D2["Certificate and public key pinning"]
  A3["Thief with unlocked device"] --> D3["Face ID or passcode gating"]
  A4["Thief with locked device and time"] --> D4["Keychain data protection classes"]
  A5["Attacker with root or forensic image"] --> D5["Secure Enclave"]
```
*The five adversaries, ranked weakest to strongest, each answered by one primitive.*
...
```mermaid
flowchart TD
  A["TLS challenge received"] --> B{"Server trust challenge"}
  B -- No --> C["Default handling"]
  B -- Yes --> D["Evaluate CA trust chain"]
  D -- Fails --> E["Cancel challenge"]
  D -- Passes --> F["Compute leaf SPKI hash"]
  F --> G{"Hash in pinned set"}
  G -- No --> E
  G -- Yes --> H["Use credential"]
  H --> I["Connection proceeds"]
```
*How the pinning delegate decides between rejecting and proceeding on each TLS challenge.*

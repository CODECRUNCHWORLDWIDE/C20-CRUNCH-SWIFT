...
```mermaid
flowchart LR
  A["Generate P256 key inside Secure Enclave"] --> B["App holds a key handle only"]
  B --> C["Persist data representation in Keychain"]
  C --> D["Encrypted blob bound to this device"]
  D --> E["Reload representation from Keychain"]
  E --> F["Enclave rehydrates the key handle"]
```
*The private key never leaves the Enclave — what round-trips through the Keychain is a device-bound encrypted representation, not the key itself.*
...
sequenceDiagram
  ...
  D->>E: Generate P256 signing key
  E-->>D: Public key and key handle
  D->>S: Enroll public key under a key ID
  D->>E: Sign canonical request string
  E-->>D: Detached signature
  D->>S: Send request with signature and timestamp
  S->>S: Rebuild canonical string and verify signature
```
*Enrollment happens once; every later request is signed by the Enclave and independently verified on the server.*

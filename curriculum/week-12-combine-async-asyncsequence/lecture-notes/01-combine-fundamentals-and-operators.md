...
```mermaid
flowchart LR
  A["Keystrokes"] --> B["debounce 300ms"]
  B --> C["removeDuplicates"]
  C --> D["map to search publisher"]
  D --> E["switchToLatest"]
  E --> F["sink updates UI"]
```
*The canonical Combine search pipeline — each operator narrows the stream before it reaches the UI.*
...
```mermaid
flowchart TD
  A["Publisher work"] --> B["subscribe on background queue"]
  B --> C["map expensive transform"]
  C --> D["receive on main thread"]
  D --> E["sink updates UI"]
```
*subscribe-on picks the upstream queue; receive-on hands delivery off to the main thread before the sink touches UI.*

...
```mermaid
flowchart TD
  A["Callback source manager"] -- "onUpdate fires" --> B["continuation yield"]
  B --> C["AsyncStream buffer"]
  C --> D["for await consumer loop"]
  D -- "task cancelled" --> E["onTermination cleanup"]
  E --> F["stop manager"]
```
*A callback source is turned into a pull-based stream: yield feeds it, onTermination tears it down.*
...
```mermaid
flowchart TD
  A["What changes"] --> B{"Synchronous state"}
  B -- "Yes" --> C["Use Observable"]
  B -- "No" --> D{"Your own async stream"}
  D -- "Yes" --> E["Use task or task id"]
  D -- "No" --> F["Framework Combine publisher"]
  F --> G["Use onReceive or bridge with values"]
```
*Which SwiftUI reactivity tool to reach for, decided by what kind of change you're observing.*

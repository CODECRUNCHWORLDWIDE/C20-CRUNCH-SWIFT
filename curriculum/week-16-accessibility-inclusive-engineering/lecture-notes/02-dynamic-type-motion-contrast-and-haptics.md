...
```mermaid
flowchart TD
  A["Dynamic Type size"] --> B{"Is accessibility size"}
  B -->|Yes| C["Stack vertically"]
  B -->|No| D["Stack horizontally"]
  C --> E["Text gets full width no clipping"]
  D --> E
```
*AdaptiveRow reflows from a horizontal to a vertical layout once the user crosses into the AX accessibility size range.*
...
```mermaid
flowchart LR
  A["Lose sight"] --> B["Accessibility tree and VoiceOver"]
  C["Need bigger text"] --> D["Dynamic Type"]
  E["Cannot see color"] --> F["Shape and text cues"]
  G["Cannot tolerate motion"] --> H["Fade instead of slide"]
  I["Want confirmation without looking"] --> J["Haptics"]
```
*Every perceptual limitation has a fallback channel that carries the same meaning — the redundancy that makes a UI inclusive.*

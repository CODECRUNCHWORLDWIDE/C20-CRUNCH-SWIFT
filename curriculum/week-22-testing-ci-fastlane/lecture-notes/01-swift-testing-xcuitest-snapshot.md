...
```mermaid
flowchart TD
  A["Render SwiftUI view to image"] --> B{"Reference image exists"}
  B -- No --> C["Record baseline image"]
  B -- Yes --> D["Diff against reference pixel by pixel"]
  D --> E{"Any difference"}
  E -- No --> F["Test passes"]
  E -- Yes --> G["Test fails with side by side diff"]
```
*How a snapshot test moves from first run to a trusted pass or fail.*
...
```mermaid
flowchart TD
  A["New test needed"] --> B{"What is being checked"}
  B -- "Logic or transformation" --> C["Swift Testing"]
  B -- "Rendering or layout" --> D["Snapshot test"]
  B -- "A full user journey" --> E["XCUITest"]
  C --> F["Fast, parallel, runs on every PR"]
  D --> F
  E --> G["Slow, keep to a critical few"]
```
*Which layer a new test belongs in, based on what it needs to prove.*

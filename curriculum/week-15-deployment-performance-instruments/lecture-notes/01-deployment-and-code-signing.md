...
```mermaid
flowchart TD
  A["Certificate - who"] --> P["Provisioning profile"]
  B["App ID - what"] --> P
  C["Entitlements - may do"] --> P
  U["Device UDID list - where"] --> P
  P --> L["Device checks at launch"]
  L --> R["App runs"]
```

*How the four signing pieces staple together into the provisioning profile the device checks at launch.*

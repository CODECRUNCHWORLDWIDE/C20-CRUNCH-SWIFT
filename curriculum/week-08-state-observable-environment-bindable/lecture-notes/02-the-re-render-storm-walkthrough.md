...
```mermaid
flowchart TD
  A["CalmListView owns store and searchText"] --> B["SearchField binds text"]
  A --> C["ResultsList takes filtered notes"]
  C --> D["CalmRow takes one note"]
```
*The repaired view tree: each level takes only the minimal input it needs, so churn stays contained.*
...
```mermaid
flowchart TD
  A["Storm suspected"] --> B{"See identity changed"}
  B -->|"yes"| C["Cause D unstable id"]
  B -->|"no"| D{"Unrelated state re-renders view"}
  D -->|"yes"| E["Cause A coarse model"]
  D -->|"no"| F{"Sibling data change re-renders view"}
  F -->|"yes"| G["Cause B over broad inputs"]
  F -->|"no"| H{"High frequency input hits expensive tree"}
  H -->|"yes"| I["Cause C scope"]
```
*Walking the four-cause checklist of section 2.8 in diagnostic order.*

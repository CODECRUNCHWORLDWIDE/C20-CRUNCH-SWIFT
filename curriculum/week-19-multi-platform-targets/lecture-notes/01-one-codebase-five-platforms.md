...
```mermaid
flowchart TD
    Core["NotesCore package - shared, platform agnostic"]
    Core --> IOSTarget["NotesPro iOS - iPhone iPad Mac"]
    Core --> WatchTarget["NotesPro Watch - own main"]
    Core --> VisionTarget["NotesPro Vision"]
    Core --> WidgetTarget["Watch Complication - own main"]
```

*Every platform shell depends on the same NotesCore package, which turns the share slash adapt line into a compiler-enforced boundary.*
...
```mermaid
flowchart TD
    Decl["One NavigationSplitView declaration"] --> Wide["Wide Mac window or iPad landscape - three columns"]
    Decl --> Narrow["Narrower iPad - two columns"]
    Decl --> Stack["iPhone - navigation stack"]
```

*The same declaration renders as three columns, two columns, or a push stack depending on available space, with zero platform branches.*

...
```mermaid
flowchart LR
  A["Plain SwiftUI logic in view"] --> B["MVVM Observable view model"]
  B --> C["Unidirectional data flow"]
  C --> D["TCA reducers effects tests"]
```
*Moving right along the axis trades cheap-to-write for cheap-to-change at scale.*
...
```mermaid
flowchart TD
  VM["NotesListModel view model"] --> P["NotesRepository protocol"]
  P --> Live["LiveNotesRepository real network"]
  P --> Stub["StubRepository test double"]
```
*The view model depends on an abstraction, not a concretion, so a test can swap in a stub.*

...
```mermaid
flowchart LR
  A["Action sent to store"] --> B["Reducer mutates State"]
  B --> C["View renders State"]
  B --> D["Reducer returns Effect"]
  D --> E["Store runs Effect"]
  E --> A
```
*Every action flows through the reducer once, and any effect eventually feeds a new action back in.*
...
```mermaid
flowchart LR
  V["View passive UI"] --> P["Presenter middleman"]
  P --> I["Interactor business logic"]
  I --> Ent["Entity plain model"]
  P --> R["Router navigation"]
```
*Five VIPER components wired together by protocols, one full set per screen.*

...
```mermaid
flowchart TD
  A["Parent task cancelled"] --> B["Cancellation flag set on every descendant"]
  B --> C["Child task 1 checks flag at next suspension"]
  B --> D["Child task 2 checks flag at next suspension"]
  B --> E["Child task 3 checks flag at next suspension"]
  C --> F["Throws CancellationError"]
  D --> F
  E --> F
  F --> G["Tree drains clean"]
```
*Cancelling a parent marks every descendant, and each one stops cooperatively at its own next suspension point.*
...
```mermaid
flowchart TD
  A["Prime window with N tasks"] --> B["Wait for next task to finish"]
  B --> C["Collect result"]
  C --> D{"More URLs left"}
  D -- Yes --> E["Start one new task"]
  E --> B
  D -- No --> F["Window drains to empty"]
  F --> G["Return all results"]
```
*The sliding window keeps at most N requests in flight, starting a new one only when an old one finishes.*

...
Do the [mini-project](../mini-project/README.md) first, or at least scaffold its package and get the basic fan-out working. This challenge adds a feature *on top of* the link-checker — it assumes you already have `check(url:)`, a bounded `TaskGroup`, a `--concurrency` flag, a `--timeout`, and the `SIGINT` trap from the mini-project.

...
sequenceDiagram
    ...
    participant Cache as Cache Actor
    Main->>Cache: await cache.get feed - HOP 1
    Note over Cache: Runs get on cache executor
    Cache-->>Main: return raw data - HOP 2
    Note over Main: Resume refresh and mutate items
```
*One await crosses the water twice: out to the cache actor and back to main.*
...
sequenceDiagram
    ...
    A->>L: check cache for url
    L-->>A: not found
    A->>L: await download starts
    Note over L: A suspended - actor free to accept B
    B->>L: check cache for url
    L-->>B: not found
    B->>L: await download starts
    Note over L: both downloads run - duplicate work
    A->>L: write cache
    B->>L: write cache
```
*Reentrancy lets Call B start before Call A's download writes back, so both fetch the same url.*

...
flowchart TD
  ...
  subgraph Widget["Widget extension process"]
    W["Widget extension target"]
  end
  S["Shared App Group container Notes dot store"]
  A -->|reads and writes| S
  W -->|reads and writes| S
```
*The app and the widget extension are separate processes that only meet through the shared App Group container.*
...
```mermaid
flowchart LR
  P["placeholder instant fake data"] --> G["getSnapshot one entry for gallery"]
  G --> T["getTimeline entries plus reload policy"]
  T --> R["System renders each entry at its date"]
  R -->|policy is never| X["Waits for an explicit reload"]
  X -->|WidgetCenter reloadTimelines| T
```
*The provider lifecycle: entries render on schedule, and an app-triggered reload restarts the cycle.*

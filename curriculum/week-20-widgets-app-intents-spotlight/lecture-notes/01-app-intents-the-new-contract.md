...
```mermaid
flowchart TD
  A["NoteEntity"] --> B["defaultQuery: NoteQuery"]
  B --> C["entities for id list"]
  B --> D["suggestedEntities"]
  B --> E["entities matching string"]
  C --> F["Resolves a piped value"]
  D --> G["Default picker list"]
  E --> H["Free text search in picker"]
```
*How EntityQuery resolves an App Entity by id, by suggestion, and by search text.*
...
```mermaid
flowchart TD
  A["Intent finishes its work"] --> B{"Does the user need the full UI"}
  B -->|No| C["Return a dialog headless"]
  B -->|A glance helps| D["Return a snippet view"]
  B -->|Yes take me into the app| E["Set openAppWhenRun true"]
```
*Choosing between a headless dialog, a snippet view, or opening the app.*

...
```mermaid
flowchart TD
  A["buildRequest from Endpoint"] --> B["session data for request"]
  B --> C{"URLError thrown"}
  C -- Yes --> D["NetworkError transport"]
  C -- No --> E{"Cast to HTTPURLResponse"}
  E -- Fail --> D
  E -- OK --> F{"Status 200 to 299"}
  F -- No --> G["NetworkError http"]
  F -- Yes --> H["Decode Response"]
  H -- Fail --> I["NetworkError decoding"]
  H -- OK --> J["Return typed Response"]
```
*How NotesClient.send turns a request into a typed Response or one of the typed errors.*
...
```mermaid
flowchart TD
  A["NetworkError case"] --> B{"Which case"}
  B -- offline --> C["Show cached notes"]
  B -- cancelled --> D["Show nothing"]
  B -- http 401 --> E["Refresh token and retry"]
  B -- http 5xx --> F["Show retry banner"]
  B -- decoding --> G["Show update app banner"]
```
*How the caller branches on the kind of NetworkError instead of treating every failure the same.*

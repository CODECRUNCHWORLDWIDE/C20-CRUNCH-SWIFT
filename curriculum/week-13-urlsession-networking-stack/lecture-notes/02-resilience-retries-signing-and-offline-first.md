...
```mermaid
flowchart TD
  A["send endpoint"] --> B{"Threw NetworkError"}
  B -- No --> C["Return response"]
  B -- Yes --> D{"Retryable and attempts left"}
  D -- No --> E["Throw error"]
  D -- Yes --> F["Wait backoff with jitter or Retry-After"]
  F --> A
```
*The sendWithRetry loop: only retryable errors under the attempt cap get another pass, after a jittered wait.*
...
sequenceDiagram
  participant User
  participant Local as SwiftData Local
  participant Outbox
  participant Server

  User->>Local: Apply change locally
  Local->>Outbox: Append PendingMutation
  Note over Outbox: Offline, user keeps working
  Outbox->>Server: Replay mutations in order on reconnect
  Server-->>Outbox: Success, remove mutation
  Server-->>Outbox: Retryable failure, stop and retry later
```
*The offline-first outbox: writes apply locally first, then drain to the server in order once connectivity returns.*

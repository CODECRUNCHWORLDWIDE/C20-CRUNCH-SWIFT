...
```mermaid
flowchart TD
  A["Binding constraint"] --> B["Shared types with a Swift client"]
  A --> C["Smallest concurrency-native surface"]
  A --> D["ML or data adjacent, Python-native team"]
  A --> E["Need OpenAPI docs today, free"]
  A --> F["Maximal flexibility, npm ecosystem"]
  A --> G["Fastest idea to CRUD, Ruby team"]
  B --> H["Vapor"]
  C --> I["Hummingbird"]
  D --> J["FastAPI"]
  E --> J
  ...
  G --> L["Rails"]
```
*Matching a team's binding constraint to the framework it points toward.*

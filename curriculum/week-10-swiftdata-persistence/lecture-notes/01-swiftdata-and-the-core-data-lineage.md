...
```mermaid
flowchart TD
  A["SwiftUI - Query and modelContext"] --> B["SwiftData - Model and ModelContainer"]
  B --> C["Core Data - NSManagedObjectContext"]
  C --> D["SQLite - store file on disk"]
```
*Every SwiftData call passes down through Core Data to the same SQLite file.*
...
```mermaid
flowchart TD
  A["Delete a Note"] --> B{"Which delete rule"}
  B -->|"nullify"| C["Keep the Tag - clear the link"]
  B -->|"cascade"| D["Delete the child Tag too"]
  B -->|"deny"| E["Refuse the delete if links exist"]
  B -->|"noAction"| F["Leave a dangling reference"]
```
*Choosing a delete rule is choosing what happens to related rows.*

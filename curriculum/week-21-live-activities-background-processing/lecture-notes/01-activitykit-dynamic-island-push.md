...
stateDiagram-v2
  [*] --> Requested
  Requested --> Active: Activity dot request pushType token
  Active --> Active: update ContentState
  Active --> Ended: end with dismissalPolicy
  Ended --> [*]
```
*The three calls that drive a Live Activity from request through updates to end.*
...
```mermaid
sequenceDiagram
  participant DeviceB as Device B editor
  participant Backend as Vapor backend
  participant APNs
  participant DeviceA as Device A lock screen

  DeviceB->>Backend: start editing note X
  Backend->>APNs: push event start to push-to-start token
  APNs->>DeviceA: deliver start push
  DeviceA->>DeviceA: iOS creates the Live Activity
  Backend->>APNs: push event update with content state
  APNs->>DeviceA: deliver update push
  DeviceA->>DeviceA: lock screen shows new state
```
*A backend event travels through APNs to start, then update, a Live Activity while the app is closed.*

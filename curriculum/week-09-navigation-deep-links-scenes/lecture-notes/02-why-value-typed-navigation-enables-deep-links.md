...
sequenceDiagram
  ...
  alt App already running
    iOS->>App: Foreground and deliver URL
    App->>App: onOpenURL runs, path is set
  else App was killed
    iOS->>App: Launch process and build view tree
    iOS->>App: Deliver URL once scene is ready
    App->>App: onOpenURL runs, path is set
  end
```
*Warm and cold launch both end at the same onOpenURL call — one handler, two lifecycles.*
...
```mermaid
flowchart TD
  A["Cold launch delivers URL"] --> B{"Is onOpenURL on the always-present root"}
  B -->|"No - attached to a conditional child"| C["Child view does not exist yet"]
  C --> D["URL dropped silently - the bug"]
  B -->|"Yes - attached above the conditional"| E["Store URL in pendingURL"]
  E --> F["Store finishes loading"]
  F --> G["Apply the buffered URL - the fix"]
```
*Where the handler is attached decides whether a cold-launch link survives or gets dropped.*

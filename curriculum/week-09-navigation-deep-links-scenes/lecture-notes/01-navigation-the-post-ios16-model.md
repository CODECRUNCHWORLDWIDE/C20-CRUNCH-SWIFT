...
stateDiagram-v2
  Root --> OneDeep : push
  OneDeep --> TwoDeep : push
  TwoDeep --> OneDeep : pop
  OneDeep --> Root : pop
  Root --> TwoDeep : replace
  TwoDeep --> Root : pop to root
```
*The path array as a state machine — every navigation operation is a mutation between depths.*
...
```mermaid
flowchart LR
  subgraph Wide["iPad or Mac - regular width"]
    SB["Sidebar column"] --> CT["Content column"] --> DT["Detail column"]
  end
  subgraph Narrow["iPhone - compact width"]
    S2["Sidebar becomes root screen"] --> C2["Row tap pushes Content"] --> D2["Row tap pushes Detail"]
  end
```
*The same three-column layout renders side by side on iPad or Mac and collapses into one pushed stack on iPhone.*

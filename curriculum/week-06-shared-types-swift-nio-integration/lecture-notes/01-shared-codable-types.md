...
```mermaid
flowchart LR
  NC["NotesCore library"]
  API["notes-api server"]
  CLI["notes-cli client"]
  API -->|imports| NC
  CLI -->|imports| NC
```
*Both the server and the client depend on the same NotesCore module — one Note type, two consumers.*
...
```mermaid
flowchart LR
  DB["Postgres row"] --> Model["NoteModel Fluent class"]
  Model -->|toDTO| Wire["Note wire type"]
  Wire -->|retroactive Content| HTTP["HTTP JSON response"]
```
*The Fluent model never crosses the wire — toDTO projects it onto the shared Note type at the route boundary.*
